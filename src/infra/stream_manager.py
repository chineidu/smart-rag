...
class StreamSessionManager:
    ...
    send_event(session_id: str, event_type: str, data: Any, metadata: dict[str, Any] | None = None) -> bool
        Sends an event to the Redis stream for the given session.
    read_events(session_id: str, last_id: str = "0", block: int = 5_000, count: int = 100)
    ...
    async def read_events(
        self, session_id: str, last_id: str = "0", block: int = 5_000, count: int = 100
    ) -> AsyncGenerator[dict[str, Any], None]:
        """Reads events from the Redis stream for the given session.
            ...
            try:
                ...
                for _, messages in response:
                    for message_id, message_data in messages:
                        ...
                        if event["event_type"] == EventsType.SESSION_ENDED:
    ...
    def remove_connection(self, session_id: str) -> None:
        ...
        """
        if (
            session_id in self._active_connecton
